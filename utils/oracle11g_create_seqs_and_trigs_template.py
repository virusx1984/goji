# /goji/utils/oracle11g_create_seqs_and_trigs_template.py

# Revision identifiers, used by Alembic.
revision = 'xxxx_add_oracle_sequences' # Replace with your actual revision ID
down_revision = 'yyyy'  # Replace with the previous migration's revision ID
branch_labels = None
depends_on = None

"""
! 这个文件用来记录oracle 11g创建sequence和trigger的migration py文件，将下列
! 代码复制粘贴到用flask db revision -m "some message" 创建的migration py文件里面
! 注意贴在 revision, down_revisiion, branch_labels, depends_on变量下面，同时删除上面的import
"""
...
#! 注意一下三个import vscode 中无法解析是没问题的，运行时候不会错，不用改
